...
export default function Dashboard() {
  ...
  const cardProps = (task) => ({
    ...
    statusBusy: statusBusyId === task.id,
  });
      ...
      <section aria-labelledby="due-soon-title">
        ...
        {dueSoonTasks.length > 0 ? (
          <div className="space-y-2 md:hidden">
            {dueSoonTasks.map((t) => (
              <DeadlineCard key={`${t.id}-${t._occurrenceDate || 'base'}`} {...cardProps(t)} />
            ))}
          </div>
        ) : (
          <DueSoonEmpty />
        ...
        {dueSoonTasks.length > 0 && (
          <div className="hidden md:block space-y-2">
            <AnimatePresence>
              {dueSoonTasks.map((t) => (
                <TaskCard key={`${t.id}-${t._occurrenceDate || 'base'}`} {...cardProps(t)} />
              ))}
            </AnimatePresence>
          </div>
        )}
      ...
      {recentDone.length > 0 && (
        <section id="section-concluidas" aria-labelledby="done-title">
          ...
          <div className="space-y-2">
            <AnimatePresence>
              {recentDone.map((t) => (
                <TaskCard key={`${t.id}-${t._occurrenceDate || 'done'}`} {...cardProps(t)} compact />
              ))}
            </AnimatePresence>
          </div>
        </section>
